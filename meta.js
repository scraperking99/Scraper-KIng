...
const keepAliveAgent = new https.Agent({ keepAlive: true, maxSockets: 200, timeout: 100000 });
...
function printHeader() {
    ...
    console.log(W('│ [•] Version   : ') + Y('V7.0.1                       ') + W('│'));
    console.log(W('└──────────────────────────────────────────────┘\n'));
}
...
// ── Device profile database (sourced from mob 8-22-26, log ua & OTP buster captures) ──
const ANDROID_DEVICE_PROFILES = [
    // Qualcomm Snapdragon flagship & mid
    ...
    { model: 'SM-S928B', cpu: 'Snapdragon 8 Gen 3', gpu: 'Adreno 750', ram: 12, dpr: 3.0, group: 'high', soc: 'Qualcomm' },
    ...
    { model: 'vivo 1919', cpu: 'Snapdragon 665', gpu: 'Adreno 610', ram: 8, dpr: 2.75, group: 'medium', soc: 'Qualcomm' },
    ...
    { model: '2312DRA50G', cpu: 'MediaTek Dimensity 7200-Ultra', gpu: 'Mali-G610 MC4', ram: 12, dpr: 2.75, group: 'high', soc: 'MediaTek' },
    ...
    // Google Pixel (from mob 8-22-26 capture)
    { model: 'Pixel 8', cpu: 'Google Tensor G3', gpu: 'Mali-G715 Immortalis MC10', ram: 8, dpr: 2.75, group: 'high', soc: 'Google' },
    { model: 'Pixel 8 Pro', cpu: 'Google Tensor G3', gpu: 'Mali-G715 Immortalis MC10', ram: 12, dpr: 3.0, group: 'high', soc: 'Google' },
    { model: 'Pixel 7 Pro', cpu: 'Google Tensor G2', gpu: 'Mali-G710 MP7', ram: 12, dpr: 3.5, group: 'high', soc: 'Google' },
];

const IOS_DEVICE_PROFILES = [
    // iPhone flagships & popular series (from log ua)
    ...
    { model: 'iPhone13,4', family: 'iPhone', osVer: '16.2', cpu: 'Apple A14 Bionic', gpu: 'Apple G14 (4-core)', ram: 6, dpr: 3.0, group: 'medium', soc: 'Apple' },
    { model: 'iPhone12,8', family: 'iPhone', osVer: '15.5', cpu: 'Apple A13 Bionic', gpu: 'Apple G13 (4-core)', ram: 3, dpr: 2.0, group: 'medium', soc: 'Apple' },
    // iPad (from log ua)
    ...
    { model: 'iPad13,18', family: 'iPad', osVer: '17.1', cpu: 'Apple A14 Bionic', gpu: 'Apple G14 (4-core)', ram: 4, dpr: 2.0, group: 'medium', soc: 'Apple' },
    { model: 'iPad13,1', family: 'iPad', osVer: '16.1', cpu: 'Apple A14 Bionic', gpu: 'Apple G14 (4-core)', ram: 4, dpr: 2.0, group: 'medium', soc: 'Apple' },
    { model: 'iPad11,6', family: 'iPad', osVer: '15.4', cpu: 'Apple A12 Bionic', gpu: 'Apple G12 (4-core)', ram: 3, dpr: 2.0, group: 'medium', soc: 'Apple' },
    { model: 'iPad8,11', family: 'iPad', osVer: '14.6', cpu: 'Apple A12Z Bionic', gpu: 'Apple G12Z (8-core)', ram: 6, dpr: 2.0, group: 'high', soc: 'Apple' }
];

const ANDROID_VERSIONS = ['11', '12', '13', '14', '15'];
const IOS_VERSIONS = [
    { ver: '18_5', build: '22F76', osTag: '18.5' },
    { ver: '18_3_1', build: '22D63', osTag: '18.3' },
    { ver: '18_3', build: '22D60', osTag: '18.3' },
    { ver: '18_2', build: '22C152', osTag: '18.2' },
    { ver: '18_1', build: '22B83', osTag: '18.1' },
    { ver: '18_0', build: '22A3354', osTag: '18.0' },
    { ver: '17_5', build: '21F79', osTag: '17.5' },
    { ver: '17_4_1', build: '21E236', osTag: '17.4.1' },
    { ver: '17_2', build: '21C62', osTag: '17.2' },
    { ver: '17_1', build: '21B74', osTag: '17.1' },
    { ver: '16_3', build: '20D47', osTag: '16.3' },
    { ver: '16_2', build: '20C65', osTag: '16.2' },
    { ver: '16_1', build: '20B82', osTag: '16.1' },
    { ver: '15_7', build: '19H12', osTag: '15.7' },
    { ver: '15_6', build: '19G71', osTag: '15.6' },
    { ver: '15_5', build: '19F77', osTag: '15.5' },
    { ver: '15_4', build: '19E241', osTag: '15.4' },
    { ver: '15_2', build: '19C56', osTag: '15.2' },
    { ver: '15_1', build: '19B74', osTag: '15.1' },
    { ver: '14_8_1', build: '18H107', osTag: '14.8.1' },
    { ver: '14_7_1', build: '18G82', osTag: '14.7.1' },
    { ver: '14_6', build: '18F72', osTag: '14.6' },
    { ver: '14_2', build: '18B92', osTag: '14.2' },
    { ver: '14_0', build: '18A373', osTag: '14.0' }
];
...
function getRandomClient(browserPref = 'random', simPool = null) {
    const v = Math.floor(Math.random() * (152 - 148 + 1)) + 148;
    ...
    const grease = greaseMap[v] || '"Not?A_Brand";v="24"';
    ...
    if (isDesktop) {
        ...
        if (device.platform === 'ios') {
            ...
            const osStr = isIPad ? `CPU OS ${iv.ver} like Mac OS X` : `CPU iPhone OS ${iv.ver} like Mac OS X`;
            const verDisplay = iv.osTag.split('.').slice(0, 2).join('.');
            const ua = `Mozilla/5.0 (${isIPad ? 'iPad' : 'iPhone'}; ${osStr}) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/${verDisplay} Mobile/${iv.build} Safari/605.1`;
            return {
                ...
                hw: { w: isIPad ? 768 : 390, h: isIPad ? 1024 : 844, dpr: device.dpr, cores: 8, ram: device.ram },
                device
            };
        } else {
            const ua = `Mozilla/5.0 (Linux; Android ${device.androidVersion}; ${device.model}) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/${v}.0.${buildNum}.${patch} Mobile Safari/537.36`;
            return {
                name: 'Android Chrome',
                userAgent: ua,
                clientHints: {
                    'sec-ch-ua': `${grease}, "Google Chrome";v="${v}", "Chromium";v="${v}"`,
                    'sec-ch-ua-full-version-list': `${greaseFullVer}, "Google Chrome";v="${v}.0.${buildNum}.${patch}", "Chromium";v="${v}.0.${buildNum}.${patch}"`,
                    'sec-ch-ua-mobile': '?1',
                    'sec-ch-ua-platform': '"Android"',
                    'sec-ch-ua-platform-version': `"${device.androidVersion}"`,
...
// ── 5.5 Proxy Country — extracted from proxy string (no HTTP request needed) ──
// Supports DataImpulse (__cr.XX) and OwlProxy (_zone_XX_) formats
function getProxyCountry(proxy) {
    if (!proxy) return 'Direct';
    const userStr = (proxy.user || '').toLowerCase();
    let isoCode = null;

    // DataImpulse: __cr.gb or _cr.gb
    const crMatch = userStr.match(/_cr\.([a-z]{2})\b/);
    if (crMatch) isoCode = crMatch[1].toUpperCase();

    // OwlProxy / zone-based: _zone_AE_ or _zone_AE
    if (!isoCode) {
        const zoneMatch = userStr.match(/_zone_([a-z]{2})(?:_|$)/);
        if (zoneMatch) isoCode = zoneMatch[1].toUpperCase();
    }

    // Generic residential: -country-gb, -cc-gb, customer-gb
    if (!isoCode) {
        const m = userStr.match(/[_\-]country[_\-]([a-z]{2})/i) ||
            userStr.match(/[_\-]cc[_\-]([a-z]{2})/i) ||
            userStr.match(/customer[_\-]([a-z]{2})\b/i);
        if (m) isoCode = m[1].toUpperCase();
    }

    if (!isoCode) return 'Proxy';
    try { return _displayNames.of(isoCode) || isoCode; } catch (_) { return isoCode; }
}


// ── 6. HTTP Wrapper ────────────────────────────────────────────
function sendRequest(urlStr, method, headers, postData, proxyStr = null, timeout = 100000, redirectCount = 0, retryCount = 0, flowAgent = null) {
    return new Promise((resolve, reject) => {
        ...
        // ── Agent selection: flowAgent (pre-built per-flow) takes priority ──
        // When flowAgent is set, the same TLS session is reused for ALL requests
        // in this account flow — eliminates the TLS-fingerprint variation that Meta
        // uses to detect automation (session leak fix).
        if (flowAgent) {
            reqOpts.agent = flowAgent;
        } else {
        ...
        const req = https.request(reqOpts, res => {
            ...
            res.on('end', () => {
                ...
                if (res.statusCode >= 300 && res.statusCode < 400 && res.headers.location) {
                    ...
                    resolve(sendRequest(loc, nextMethod, newHeaders, nextMethod === 'GET' ? null : postData, proxyStr, timeout, redirectCount + 1, retryCount, flowAgent).then(redirectRes => {
                ...
                if (res.statusCode === 407) {
                    return reject(new Error('Proxy error: 407 Proxy Authentication Required (check proxy credits or IP whitelist)'));
                }

                // If proxy returned an empty body on 200 or 502/503/504 gateway error, retry
                if ((!body.length || res.statusCode >= 502) && retryCount < 3) {
                    const backoff = (retryCount + 1) * 800;
                    setTimeout(() => {
                        sendRequest(urlStr, method, headers, postData, proxyStr, timeout, redirectCount, retryCount + 1, flowAgent)
        ...
        req.on('error', (err) => {
            ...
            const isProxyDrop = (
                ...
                msg.includes('EPROTO') ||
                msg.includes('bad record')
            );
            // Auto-retry up to 3 times on transient proxy CONNECT drops
            if (isProxyDrop && retryCount < 3) {
                const backoff = (retryCount + 1) * 800;
                setTimeout(() => {
                    sendRequest(urlStr, method, headers, postData, proxyStr, timeout, redirectCount, retryCount + 1, flowAgent)
                        .then(resolve).catch(reject);
...
function parseTokensFromHtml(html) {
    ...
    tokens.jazoest = extract(/name="jazoest" value="([^"]+)"/) ||
        extract(/"jazoest":\s*"([^"]+)"/) ||
        extract(/jazoest=(\d+)/);
    tokens.hsi = extract(/"hsi":\s*"([^"]+)"/) || extract(/"__hsi":\s*"([^"]+)"/);
    tokens.spin_r = extract(/"__spin_r":\s*(\d+)/) || extract(/"spin_r":\s*(\d+)/);
    tokens.spin_b = extract(/"__spin_b":\s*"([^"]+)"/) || extract(/"spin_b":\s*"([^"]+)"/);
    tokens.spin_t = extract(/"__spin_t":\s*(\d+)/) || extract(/"spin_t":\s*(\d+)/);

    // Dynamic per-session revision extraction
    tokens.rev = extract(/"server_revision":\s*(\d+)/) ||
        extract(/server_revision":\s*(\d+)/) ||
        extract(/"client_revision":\s*(\d+)/) ||
        extract(/"__rev":\s*(\d+)/) ||
        extract(/"rev":\s*(\d+)/) ||
        tokens.spin_r;

    tokens.hs = extract(/"haste_session":\s*"([^"]+)"/) ||
        extract(/"__hs":\s*"([^"]+)"/) ||
        extract(/"hs":\s*"([^"]+)"/);
    ...
    // ── Dynamic bundle tokens — extract live from Meta's page HTML ──
    // ROOT CAUSE FIX: All workers sharing the same hardcoded __dyn/__csr value
    // is Meta's #1 bot fingerprint signal. After ~50-100 accounts, Meta rate-limits
    // that bundle hash. Extract the live values from each session's actual page.
    tokens.dyn = extract(/"__dyn"\s*:\s*"([^"]+)"/) || extract(/__dyn.*?"([A-Za-z0-9_\-]{30,})"/m);
    tokens.csr = extract(/"__csr"\s*:\s*"([^"]+)"/) || extract(/__csr.*?"([A-Za-z0-9_\-]{30,})"/m);
    tokens.hsdp = extract(/"__hsdp"\s*:\s*"([^"]+)"/);  // If missing, hardcoded fallback is used
    tokens.hblp = extract(/"__hblp"\s*:\s*"([^"]+)"/);  // If missing, hardcoded fallback is used
    tokens.sjsp = extract(/"__sjsp"\s*:\s*"([^"]+)"/);  // If missing, hardcoded fallback is used
    return tokens;
}


// ── 8. Core API Logic (meta_account_creator.js) ───────────────
// Global Session Token Cache for Method 2 (Bandwidth Saver)
const _tokenCache = {
    tokens: null,
    cookies: [],
    fetchedAt: 0,
    useCount: 0,
    maxUses: 100, // refresh every 100 accounts
    ttlMs: 30 * 60 * 1000 // 30 minutes TTL
};

async function createAccount(phone, options = {}) {
    const { onStatus = () => { }, proxy = null, timeout = 100000, workerId = 0, browserPref = 'random', regionPref = 'random', method = '1' } = options;

    // ── SESSION LEAK FIX: Create ONE proxy agent for the entire account flow ──
    // Old code: createProxyAgent() called on every sendRequest() = 8 different TLS
    // sessions per account. Meta sees same cookies from 8 different TLS fingerprints
    // = instant bot detection after ~50-100 accounts.
    // New code: one agent = one persistent TLS session for all 8 API calls in the flow.
    const _parsedProxy = proxy ? parseProxy(proxy) : null;
    if (proxy && (!_parsedProxy || isNaN(_parsedProxy.port) || _parsedProxy.port <= 0)) {
        return { success: false, message: 'FATAL: Invalid proxy format detected', phone: normalizePhoneNumber(phone) };
    }
    const _flowAgent = _parsedProxy ? createProxyAgent(_parsedProxy) : keepAliveAgent;
    ...
    const isMethod2 = String(method) === '2';
    ...
    try {
        ...
        let tokens = null;
        const isCacheValid = _tokenCache.tokens &&
            _tokenCache.tokens.lsd &&
            _tokenCache.useCount < _tokenCache.maxUses &&
            (Date.now() - _tokenCache.fetchedAt) < _tokenCache.ttlMs;

        if (isMethod2 && isCacheValid) {
            tokens = { ..._tokenCache.tokens };
            updateCookies(_tokenCache.cookies);
            _tokenCache.useCount++;
            onStatus(`[1/4] 🚀 Method 2: Bandwidth Saver (Token Cache #${_tokenCache.useCount})...`);
        } else {
            // 1. Fetch tokens via HTTP GET through proxy (Method 1 or Method 2 cache refresh)
            const initStart = Date.now();
            onStatus(`[1/4] Fetching tokens (${isMethod2 ? 'Refreshing Token Cache' : 'Method 1: Fresh Session'})...`);
            const initHeaders = {
            ...
            const initRes = await sendRequest(initialUrl, 'GET', initHeaders, null, null, timeout, 0, 0, _flowAgent);
            if (!initRes.data) throw new Error(`Failed to load Meta page (HTTP ${initRes.status || 'No status'} from proxy - check proxy connection)`);

            tokens = parseTokensFromHtml(initRes.data);
            if (!tokens.lsd || !tokens.jazoest) throw new Error('Failed to extract LSD/Jazoest tokens.');

            updateCookies(initRes.cookies);

            if (isMethod2) {
                _tokenCache.tokens = tokens;
                _tokenCache.cookies = initRes.cookies || [];
                _tokenCache.fetchedAt = Date.now();
                _tokenCache.useCount = 1;
            }
        ...
        // ── Live session tokens — Aug 31, 2026 capture (8-31-26) ──
        ...
        const liveCsr = 'gkkFVSLVJnAAHdd7ON7t8OjGiS6qzFkmQnDypoLBrooV8OU0l9o4i0xU4kM158gjw0CUIDrw8O0xljydwSxLgyl0wx2iKim3i23oGAiUlG68zh801kuE1J4qtoyqu0tC4IWU6a0aTw1Iui0kS077444pj1p0Fx2U1xo2280IywoKtw2E8qwk86O';
        const liveHsdp = 'gaYdOFscw4cgyO2EW0ZovAw2HErw0vwU5u06R80awaw';
        const liveHblp = '09qiew4bCxd3E5S0xUvAw2HECfU7O0ga0fxw0Dtw0OLw3oo29w1Ji0q62e0M-1Iw71w1560Mo2Qw2pE2hw2HU0HC';
        const liveSjsp = 'gaYdOFtR2g02ixw';
        const liveRev = '1046399105';
        const liveHs = '20696.HYP%3Afrl_comet_auth_pkg.2.1...0';

        // __s is static per session
        const sessionId = genSessionId(); // generated once, reused for all calls

        const getBasePayload = (reqVal) => {
            // Use live-extracted tokens from page HTML (fall back to hardcoded if extraction failed)
            const dynVal = tokens.dyn || liveDyn;
            const csrVal = tokens.csr || liveCsr;
            const hsdpVal = tokens.hsdp || liveHsdp;
            const hblpVal = tokens.hblp || liveHblp;
            const sjspVal = tokens.sjsp || liveSjsp;
            const revVal = tokens.rev || liveRev;
            const hsVal = tokens.hs ? encodeURIComponent(tokens.hs.includes('%') ? decodeURIComponent(tokens.hs) : tokens.hs) : liveHs;
            const spinRVal = tokens.spin_r || revVal;
            return `__user=0&__a=1&__req=${reqVal}&__hs=${hsVal}&dpr=${client.hw.dpr}&__ccg=EXCELLENT&__rev=${revVal}&__s=${sessionId}&__hsi=${tokens.hsi || ''}&__dyn=${dynVal}&__csr=${csrVal}&__hsdp=${hsdpVal}&__hblp=${hblpVal}&__sjsp=${sjspVal}&__comet_req=33&lsd=${tokens.lsd}&jazoest=${tokens.jazoest || ''}&__spin_r=${spinRVal}&__spin_b=trunk&__spin_t=${tokens.spin_t || Math.floor(Date.now() / 1000)}&__jssesw=1`;
        };
        ...
        // 2. Check Contact Point — __req=k (8-31-26 capture)
        ...
        const checkRes = await sendRequest('https://auth.meta.com/api/check-contact-point-availability/', 'POST', getHeaders(), checkPayload, null, timeout, 0, 0, _flowAgent);
        ...
        if (contactError) {
            ...
            const nonceRes = await sendRequest('https://auth.meta.com/api/login-email-otp/send-nonce/', 'POST', getHeaders(), noncePayload, null, timeout, 0, 0, _flowAgent);
            ...
            if (nonceSuccess) {
                ...
                for (let r = 0; r < (options.resends || 0); r++) {
                    ...
                    let resendRes = await sendRequest('https://auth.meta.com/api/login-email-otp/send-nonce/', 'POST', getHeaders(), resendPayload, null, timeout, 0, 0, _flowAgent);
                ...
                throw new Error('Already Registered (Existing Meta Account - Login OTP not permitted)');
            }
        }
        ...
        // 3. Reg steps — exact order from 8-31-26 capture:
        //    q: check-password → s: check-date-of-birth → t: marketing → u: check-password → y: marketing
        ...
        // Step q: First check-password (8-31-26 capture __req=q)
        ...
        const passResQ = await sendRequest('https://auth.meta.com/api/check-password/', 'POST', getHeaders(), passPayloadQ, null, timeout, 0, 0, _flowAgent);
        ...
        // Step s: check-date-of-birth (8-31-26 capture __req=s — comes BEFORE marketing)
        const dobPayload = `caa_event_flow=ntm&date_of_birth=${generatedDob.full}&first_name=&has_youth_consent=false&isf=false&last_name=&phone_number=${encodeURIComponent(normalizedPhone)}&qpl_join_id=${genQplJoinId()}&reg_integrity=${regIntegrity}&source_app_id=1522763855472543&` + getBasePayload('s');
        ...
        const dobRes = await sendRequest('https://auth.meta.com/api/check-date-of-birth/', 'POST', getHeaders(), dobPayload, null, timeout, 0, 0, _flowAgent);
        ...
        // Extract updated regIntegrity from check-dob response
        try {
            ...
            if (dobData.payload && dobData.payload.regIntegrity) {
                ...
                onStatus(`[3/4] Got updated regIntegrity from DOB check...`);
            }
        } catch (e) {
            if (!(e instanceof SyntaxError)) throw e;
        }

        // Step t: First marketing opt-in (8-31-26 capture __req=t — after check-dob)
        const marketingPayloadT = `date_of_birth=${generatedDob.full}&` + getBasePayload('t');
        const marketingResT = await sendRequest('https://auth.meta.com/api/eligible-for-default-marketing-opt-in/', 'POST', getHeaders(), marketingPayloadT, null, timeout, 0, 0, _flowAgent);
        updateCookies(marketingResT.cookies);

        // Step u: Second check-password (8-31-26 capture __req=u)
        ...
        const passRes = await sendRequest('https://auth.meta.com/api/check-password/', 'POST', getHeaders(), passPayload, null, timeout, 0, 0, _flowAgent);
        ...
        // Step y: Second marketing opt-in (8-31-26 capture __req=y)
        const marketingPayload2 = `date_of_birth=${generatedDob.full}&` + getBasePayload('y');
        const marketingRes2 = await sendRequest('https://auth.meta.com/api/eligible-for-default-marketing-opt-in/', 'POST', getHeaders(), marketingPayload2, null, timeout, 0, 0, _flowAgent);
        updateCookies(marketingRes2.cookies);

        // Generated credentials
        const username = generatedName.first.toLowerCase() + '_' + generatedName.last.toLowerCase() + '_' + Math.floor(Math.random() * 9999);
        const displayName = generatedName.first;

        // 4. Finalize Profile & Save Credentials (Triggers SMS dispatch)
        onStatus(`[4/4] Finalizing profile to dispatch SMS...`);

        // Step 12: check_profile (8-31-26 capture __req=12, is_submit=false pre-validation)
        const preProfilePayload = `client_consent_timestamp=${Math.floor(Date.now() / 1000)}&display_name=${encodeURIComponent(displayName)}&foa_import_source_name=&foa_import_source_obid=&nta_disclosures_summary_cms_id=&picture_source=&tos_cms_id=957798449862312&username=${username}&caa_event_flow=ntm&csi=${csi}&source_app_id=1522763855472543&waterfall_id=${waterfall_id}&is_submit=false&` + getBasePayload('12');
        debugLog(barePhone, 'check-profile-pre-req', { url: 'https://auth.meta.com/api/kadabra/check_profile/', method: 'POST', headers: getHeaders(), payload: preProfilePayload });
        const preProfileRes = await sendRequest('https://auth.meta.com/api/kadabra/check_profile/', 'POST', getHeaders(), preProfilePayload, null, timeout, 0, 0, _flowAgent);
        debugLog(barePhone, 'check-profile-pre-res', { status: preProfileRes.status, headers: preProfileRes.headers, body: preProfileRes.data });
        updateCookies(preProfileRes.cookies);

        // Step 15: check_profile (8-31-26 capture __req=15, is_submit=true final submit)
        const profilePayload = `client_consent_timestamp=${Math.floor(Date.now() / 1000)}&display_name=${encodeURIComponent(displayName)}&foa_import_source_name=&foa_import_source_obid=&nta_disclosures_summary_cms_id=&picture_source=&tos_cms_id=957798449862312&username=${username}&caa_event_flow=ntm&csi=${csi}&source_app_id=1522763855472543&waterfall_id=${waterfall_id}&is_submit=true&` + getBasePayload('15');
        ...
        const profileRes = await sendRequest('https://auth.meta.com/api/kadabra/check_profile/', 'POST', getHeaders(), profilePayload, null, timeout, 0, 0, _flowAgent);
        ...
        const regRes = await sendRequest('https://auth.meta.com/login/device-based/kadabra-register-save-credentials/', 'POST', getHeaders(), registerPayload, null, timeout, 0, 0, _flowAgent);
        ...
        try {
            ...
            if (dtsgAsyncMatch && !tokens.fb_dtsg) tokens.fb_dtsg = dtsgAsyncMatch[1];
        } catch (_) { }
        ...
        try {
            ...
            if (finalData.payload) {
                if (finalData.payload.account_id || finalData.payload.uid || finalData.payload.social_profile_id) {
                    registrationConfirmed = true;
                    regResultMessage = `Registration confirmed (account_id: ${finalData.payload.account_id || finalData.payload.uid})`;
                } else if (finalData.payload.redirect_uri || finalData.payload.redirectUri || finalData.payload.redirect) {
                    registrationConfirmed = true;
        ...
        // ── Loop for requested resends (Modern GraphQL FRLResendOTPMutation) ──
        ...
        for (let r = 0; r < actualResends; r++) {
            ...
            const resendBody = new URLSearchParams({
                ...
                '__user': '0',
                '__a': '1',
                '__req': (parseInt('1j', 36) + r).toString(36),
                '__hs': decodeURIComponent(tokens.hs || liveHs),
                'dpr': '1',
                '__ccg': 'EXCELLENT',
                '__rev': tokens.rev || liveRev,
                '__s': sessionId,
                '__hsi': tokens.hsi || '',
                '__dyn': tokens.dyn || liveDyn,
                '__csr': tokens.csr || liveCsr,
                '__hsdp': tokens.hsdp || liveHsdp,
                '__hblp': tokens.hblp || liveHblp,
                '__sjsp': tokens.sjsp || liveSjsp,
                '__comet_req': '33',
                'fb_dtsg': fbDtsg,
                'jazoest': tokens.jazoest || '',
                'lsd': tokens.lsd,
                '__spin_r': tokens.spin_r || tokens.rev || liveRev,
                '__spin_b': 'trunk',
            ...
            let resendRes = await sendRequest('https://auth.meta.com/api/graphql/', 'POST', resendHeaders, resendBody, null, timeout, 0, 0, _flowAgent);
        ...
        if (isMethod2 && (error.message.includes('token') || error.message.includes('LSD') || error.message.includes('400') || error.message.includes('jazoest'))) {
            _tokenCache.tokens = null;
        }
    ...
    async function interactiveWizard() {
        ...
        while (true) {
            ...
            if (step === 1) {
                ...
                // Step 8: Registration Method (Bandwidth Optimization)
                const methodChoice = await selectOption("SELECT REGISTRATION METHOD", [
                    { name: "⚡ Method 1: Standard ", value: "1" },
                    { name: "🚀 Method 2: Bandwidth Saver ", value: "2" },
                    { name: "⬅️ Go Back to OTP Resends", value: "back" }
                ]);

                if (methodChoice === 'back') {
                    step = 7;
                    continue;
                }
                regMethod = methodChoice;
                step = 9; // Done
            } else if (step === 9) {
                break;
            }
        ...
        process.stdin.pause();
        return { numbersFile, threads: String(threads), proxiesFile, region, browser, resends: String(resends), regMethod, nexaConfig, twoOoConfig, smsBowerConfig, zenexConfig };
    }
...
-----END PUBLIC KEY-----`;
    ...
    async function start() {
        ...
        if (args.length === 0) {
            ...
            args = [wiz.numbersFile, wiz.threads, wiz.proxiesFile, wiz.region, wiz.browser, 'SKING-UI-HARDWARE-ID', wiz.resends, wiz.regMethod];
            regMethodOpt = wiz.regMethod || '1';
        ...
        console.log(C(`  ✓ Method : ${regMethodOpt === '2' ? 'Method 2 (Bandwidth Saver 🚀)' : 'Method 1 (Standard ⚡)'}`));
        ...
        while (runningTool) {
            ...
            async function processWorker(workerId) {
                while (true) {
                    ...
                    let attempt = 0;
                    const maxAttempts = proxies.length > 0 ? 3 : 1;
                    let lastResult = null;
                    ...
                    while (attempt < maxAttempts) {
                        ...
                        try {
                            ...
                            if (result.success) {
                                ...
                                const isProxyDrop = result.message && (
                                    result.message.includes('CONNECT') ||
                                    result.message.includes('ended before') ||
                                    result.message.includes('socket hang up') ||
                                    result.message.includes('ECONNRESET') ||
                                    result.message.includes('ETIMEDOUT')
                                );

                                if (isProxyDrop && attempt < maxAttempts) {
                                    dashboard.addLog(`Proxy dropped on ${phone} (${attempt}/${maxAttempts}) — rotating proxy...`, 'retry', metaInfo);
                                    await sleep(1000);
                                    continue;
                                }
                            ...
                            const isProxyDrop = (
                                (err.message && (
                                    ...
                                    err.message.includes('ended before') ||
